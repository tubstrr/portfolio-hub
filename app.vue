<script setup>
  ...
  for (const category of categories) {
   for (const project of category.projects) {
    if (project.video_url) {
     video_assets.push(`https://assets.jonknoll.dev/portfolio/${project.video_url}`);
 ...
 const updateAnimationState = () => {
  ...
  window.animationState.state = states.find(([condition]) => condition)?.[1] || 'showing-hero';
   ...
   switch (to) {
    case 'bio':
     ...
     if (distanceToGo < 0) {
      ...
      const startPixels = screenHeight + screenHeight + projectsBlockRect.height - quarterScreen;
      const endPixels = screenHeight + quarterScreen; // 3500~
      const normalize = getPercentage(window.scrollY - startPixels, endPixels);
    ...
    case 'projects':
     ...
     if (distanceToGo < 0) {
      ...
      const endPixels = heroBlockRect.height + screenHeight - threeFourthsScreen;
      const normalize = getPercentage(window.scrollY - startPixels, endPixels);
      window.animationState.progress = Math.max(0, normalize);
     }
     break;
  ...
  meta: [
   {
    name: 'description',
    content:
     'Jonathan Knoll is a full stack software engineer with 10+ years of experience building fast, scalable web applications. From leading dev teams to shipping work for Fortune 500 clients, he specializes in React, Python, WordPress, and cloud infrastructure. Available for remote opportunities.',
   },
  ],
  link: [{ rel: 'preconnect', href: 'https://assets.jonknoll.dev' }],
 });
</script>
...
<style lang="scss">
 ...
 :root {
  --vw: 1vw;
  --vh: calc((100vh - env(safe-area-inset-bottom) - env(safe-area-inset-top)) / 100);
 }
