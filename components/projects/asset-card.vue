<script setup>
 ...
 const allowVideo = computed(() => {
  ...
  const hasVideo = checkURL(`https://assets.jonknoll.dev/portfolio/${video_url}`);
 ...
 $bus.on('asset-resolved', (payload) => {
  if (payload === `https://assets.jonknoll.dev/portfolio/${video_url}`) showVideo.value = true;
 });
...
<template>
 <div class="asset-card">
  <video v-if="allowVideo" muted autoplay loop :poster="`/projects/${mp4ToWebP(video_url)}`" playsinline>
   <source :src="`https://assets.jonknoll.dev/portfolio/${video_url}`" type="video/mp4" />
  </video>
